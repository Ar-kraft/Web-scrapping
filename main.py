...
# определяем url и ключевые слова для поиска
...
# получаем страницу с новыми постами и подключаем парсер
...
for article in articles:
    posts = article.find_all(class_='tm-article-snippet__title-link')
    posts = set(post.text.strip() for post in posts)

# ищем статьи, в которых есть нужные хабы
    for keyword in KEYWORDS:
        if keyword in KEYWORDS:
            if article.text.lower().find(keyword) > 0:
                ...
                print(f'{time}-{header}-{"https://habr.com" + link}')
